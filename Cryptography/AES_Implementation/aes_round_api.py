...
def add_round_key(input_state, round_key):
    ...
    for i in range(len(input_state)):
        output_state[i]=input_state[i]^round_key[i]
...
def shift_rows(input_state, mode):
    ...
    output_state = rotate(input_state,shift)  
...
def mix_column(input_state, mode):
    ...
    output_matrix = column_matrix * input_matrix
...
def aes_round_dec(input_state, round_key, last_round=False):
    ...
    output_state =  shift_rows(input_state,mode='D')
    
    # perform inverse substitute byte
    output_state = sub_bytes(output_state,mode='D')
