...
def expand_key_128(initial_key):
    ...
    while len(expanded_key) < 176:
        ...
        for fev in range(0,4):
            expanded_key.append(expanded_key[len(expanded_key)-16] ^ temp[fev])
            #print(expanded_key)   
    return expanded_key
...
def expand_key_192(initial_key):
    ...
    while len(expanded_key) < 208:
        ...
        for fev in range(0,4):
            expanded_key.append(expanded_key[len(expanded_key)-24] ^ temp[fev])    
...
def expand_key_256(initial_key):
    ...
    while len(expanded_key) < 240:
        ...
        if len(expanded_key) % 32 == 0:
            ...
            temp = g(temp,i)
            i = i + 1
        temptemp = bytearray(4)
        for geo in range (0,4):
            temptemp[geo]=sbox[temp[geo]]
            
        for fev in range(0,4):
            expanded_key.append(expanded_key[len(expanded_key)-32] ^ temptemp[fev])    
